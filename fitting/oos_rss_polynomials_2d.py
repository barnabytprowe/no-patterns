...
from polynomials_2d import (
    NX, FIT_DEGREES, X0X1_MIN, X0X1_MAX, NOISE_SIGMA, COEFF_SIGNAL_TO_NOISE, PROJDIR
)
...
# Gather fit degrees and construct display titles
DEGREE_TITLES = {
    _d: (
    ...
    for _d, _dval in FIT_DEGREES.items()
}
...
def _fit_predict(data_flat, design_matrix, nx=NX, order="C"):
...
def build_regression_sample(
    ...
    # Design matrices
    for _d, _dval in fit_degrees.items():
        output[f"features_{_d}"] = polynomials_2d.chebyshev_design_matrix(x0, x1, degree=_dval)
    ...
    output["ctrue"] = rng.normal(
        loc=0.,
        scale=noise_sigma * coeff_signal_to_noise,
    ...
    for _d in fit_degrees:
        _design_matrix = output[f"features_{_d}"]
...
if __name__ == "__main__":
    ...
    ins_residuals = {_d: results["zdata"] - results[f"pred_{_d}"] for _d in FIT_DEGREES}
    oos_residuals = {_d: results["zdata_new"] - results[f"pred_{_d}"] for _d in FIT_DEGREES}
    ins_rssn = pd.DataFrame({_d: (ins_residuals[_d]**2).mean(axis=(-2, -1)) for _d in FIT_DEGREES})
    oos_rssn = pd.DataFrame({_d: (oos_residuals[_d]**2).mean(axis=(-2, -1)) for _d in FIT_DEGREES})
